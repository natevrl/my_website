import { v4 as uuidv4 } from 'uuid';

const ProjectsDb = [{
	"id": uuidv4(),
	"title": "Minishell",
	"description": "Reproduction du fonctionnement d'un shell bash, exécution de commandes, gestions des signaux, redirections, multiples pipes...",
	"link": "https://github.com/natevrl/minishell",
	"tags": ["All", "C", "Unix/Bash"],
	"stack": ["linked lists", "unix signals", "execve"]
},
{
	"id": uuidv4(),
	"title": "Cub3d",
	"description": "Développement d'un FPS 3D en C inspiré du célèbre Wolfenstein 1992, résolue grace à la technique du ray-casting",
	"link": "https://github.com/natevrl/cub3d",
	"tags": ["All", "C"],
	"stack": ["linked lists", "raycasting algorithm"]

},
{
	"id": uuidv4(),
	"title": "Stier Movies",
	"description": "Recherchez et ajoutez vos films préférés en favoris. Coder en React.JS, feature : fetching API, inifinite scroll, redux et local storage",
	"link": "https://github.com/natevrl/StierMovies",
	"tags": ["All", "Web"],
	"stack": ["reactJS", "redux", "API", "router v6", "sass", "mui"]

},
{
	"id": uuidv4(),
	"title": "nathanvrl.tech",
	"description": "Première version de ce site codé en React.JS, avec des features comme un système de filtrage, back-end, des animations, du responsive design...",
	"link": "https://github.com/natevrl/natevrl.github.io",
	"tags": ["All", "Web"],
	"stack": ["reactJS", "router v6", "sass", "mui", "gsap", "back-end"]

},
{
	"id": uuidv4(),
	"title": "Minitalk",
	"description": "Tchat client/serveur en utilisant seulement 2 signaux UNIX (SIGUSR1 et SIGUSR2) pour envouyer un message",
	"link": "https://github.com/natevrl/minitalk",
	"tags": ["All", "C", "Unix/Bash"],
	"stack": ["linked lists", "unix signals", "execve"]

},
{
	"id": uuidv4(),
	"title": "so_long",
	"description": "Jeu vidéo 2D en C",
	"link": "https://github.com/natevrl/so_long",
	"tags": ["All", "C"],
	"stack": ["linked lists", "unix signals", "execve"]

},
{
	"id": uuidv4(),
	"title": "push_swap",
	"description": "Algorithme de tri utilisant deux stack avec un ensemble limité d'instructions",
	"link": "https://github.com/natevrl/push_swap",
	"tags": ["All", "C"],
	"stack": ["linked lists", "unix signals", "execve"]
},
{
	"id": uuidv4(),
	"title": "get_next_line",
	"description": "",
	"link": "https://github.com/natevrl/get_next_line",
	"tags": ["All", "C"],
	"stack": ["linked lists", "unix signals", "execve"]

},
{
	"id": uuidv4(),
	"title": "ft_printf",
	"description": "Reproduction de la fonction printf()",
	"link": "https://github.com/natevrl/ft_printf",
	"tags": ["All", "C"],
	"stack": ["linked lists", "unix signals", "execve"]

},
{
	"id": uuidv4(),
	"title": "philosophers",
	"description": "Projet multi-threading",
	"link": "https://github.com/natevrl/philosophers",
	"tags": ["All", "C"],
	"stack": ["linked lists", "unix signals", "execve"]

},
{
	"id": uuidv4(),
	"title": "42_POOL",
	"description": "Épreuve de selection pour entrer à l'école 42. 1 Mois hyper intense à coder en C, en P2P learning",
	"link": "https://github.com/natevrl/42_POOL",
	"tags": ["All", "C", "Unix/Bash"],
	"stack": [""]

},
{
	"id": uuidv4(),
	"title": "libft",
	"description": "Coder une librairie de plus de 50 fonctions en C",
	"link": "https://github.com/natevrl/42-libft",
	"tags": ["All", "C"],
	"stack": ["linked lists", "unix signals", "execve"]

},
{
	"id": uuidv4(),
	"title": "blacksmith_",
	"description": "Site web modern statique",
	"link": "https://github.com/natevrl/Blacksmith_",
	"tags": ["All", "Web"],
	"stack": ["HTML/CSS", "Sass"]

},
{
	"id": uuidv4(),
	"title": "born2beroot",
	"description": "Administrer et sécuriser un serveur Linux",
	"link": "https://github.com/natevrl/born2beroot",
	"tags": ["All", "Unix/Bash"],
	"stack": ["linked lists", "unix signals", "execve"]

},
{
	"id": uuidv4(),
	"title": ".vimrc",
	"description": "setup VIM",
	"link": "https://github.com/natevrl/.vimrc",
	"tags": ["All", "Unix/Bash"],
	"stack": ["linked lists", "unix signals", "execve"]

}];

export default ProjectsDb;
